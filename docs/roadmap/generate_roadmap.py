    ...
    "phases": [
        {
            ...
            "epics": [
                ...
                epic(
                    "phase-0-schema",
                    "Postgres schema v1",
                    "Atlas declarative schema, generated migrations, and sqlc foundation for core domain tables per doc 08 bootstrap step 3.",
                    [
                        task(
                            "p0-atlas-migrations-setup",
                            "Wire Atlas declarative schema and migration tooling on API startup",
                            "Add services/api/schema/ (canonical Atlas HCL) and services/api/migrations/ (generated via atlas migrate diff); API applies migrations on startup guarded by Postgres advisory lock (single-node MVP). Replace prior goose bootstrap.",
                            [
                            ...
                            spec_refs=["08-implementation-decisions-and-conventions#pinned-tool-choices", "08-implementation-decisions-and-conventions#database-schema-workflow"],
                        ),
                        task(
                            "p0-schema-core-entities",
                            "Schema: organizations, teams, users, team_memberships",
                            "Add tables to canonical Atlas schema: organizations, teams, users, team_memberships with UUIDv7 PKs, timestamptz created_at/updated_at, snake_case naming. Generate migration via atlas migrate diff — no hand-written DDL.",
                            [
                            ...
                            depends_on=["p0-atlas-migrations-setup"],
                            estimated_size="M",
                        ...
                        task(
                            ...
                            "Schema: services, integration_keys",
                            "Add services and integration_keys tables to canonical Atlas schema; integration key stores hashed token, display prefix, plugin name, JSON config, revoked_at. Generate migration via atlas migrate diff — no hand-written DDL.",
                            [
                        ...
                        task(
                            ...
                            "Schema: schedules, rotations, overrides",
                            "Add schedules (IANA timezone), rotations (RRULE text, layer, participants JSON), overrides (start/end, user swap, audit fields) to canonical Atlas schema. Generate migration via atlas migrate diff — no hand-written DDL.",
                            [
                        ...
                        task(
                            ...
                            "Schema: escalation_policies, steps, alerts, notification_attempts",
                            "Add escalation_policies, escalation_steps (ordered, delay_minutes, repeat config), alerts (status, dedup_key, escalation_state JSON), notification_attempts to canonical Atlas schema. Generate migration via atlas migrate diff — no hand-written DDL.",
                            [
                        ...
                        task(
                            ...
                            "Schema: sessions, audit_events, refresh_tokens",
                            "Add server-side sessions (HttpOnly cookie backing), audit_events append-only, refresh_tokens for mobile devices with revocation to canonical Atlas schema. Generate migration via atlas migrate diff — no hand-written DDL.",
                            [
                ...
                epic(
                    ...
                    [
                        task(
                            ...
                            depends_on=["p0-atlas-migrations-setup", "p0-go-service-skeletons"],
                            estimated_size="M",
    ...
    return [
        epic(
            ...
            [
                ...
                task(
                    "p2-mobile-server-config",
                    "In-app Escalite server URL configuration",
                    "First-run setup stores the operator's public Escalite origin in SecureStore, probes /healthz, and drives runtime API/web/GraphQL URLs for auth and push.",
                    [
                        "Unconfigured app shows server setup screen before sign-in",
                        "Saved origin persists across app restarts",
                        "Change server signs out and clears stored origin",
                    ],
                    depends_on=["p2-mobile-expo-scaffold"],
                    estimated_size="M",
                    labels=["phase-2", "mobile", "config"],
                    spec_refs=["03-mobile-app-spec#server-configuration"],
                ),
                task(
                    ...
                    depends_on=["p2-mobile-server-config", "p0-auth-login-sessions"],
                    estimated_size="M",
