...
// File is the audit record of GoAlert source IDs to Escalite UUIDs.
type File struct {
...
type Store struct {
	...
	Alerts         map[string]uuid.UUID
	RotationSchedules map[string]uuid.UUID // GoAlert rotation ID → Escalite schedule ID
...
func NewStore(orgID, teamID uuid.UUID) *Store {
	return &Store{
		...
		RotationSchedules: make(map[string]uuid.UUID),
	}
}
...
func (s *Store) ToFile(importedAt time.Time) File {
	return File{
		...
		Alerts:          uuidMapToString(s.Alerts),
		Skipped:         s.Skipped,
	}
}
