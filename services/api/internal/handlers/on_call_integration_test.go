...
func TestGraphQLOnCallNowReturnsPrimaryAndSecondaryLayers(t *testing.T) {
	...
	require.Equal(t, 200, springRec.Code, springRec.Body.String())
	...
	require.NoError(t, json.Unmarshal(springRec.Body.Bytes(), &springResp))
	require.Equal(t, scheduleID, springResp.Data.OnCallNow.ScheduleID)
	require.Len(t, springResp.Data.OnCallNow.Layers, 2)

	layersByNumber := map[int]string{}
	for _, layer := range springResp.Data.OnCallNow.Layers {
		layersByNumber[layer.Layer] = layer.UserID
	}
	require.Equal(t, admin.ID.String(), layersByNumber[1])
	require.Equal(t, userC.ID.String(), layersByNumber[2])
	...
	require.Equal(t, 200, fallRec.Code, fallRec.Body.String())
	...
	require.NoError(t, json.Unmarshal(fallRec.Body.Bytes(), &fallResp))
	require.Len(t, fallResp.Data.OnCallNow.Layers, 2)

	layersByNumber = map[int]string{}
	for _, layer := range fallResp.Data.OnCallNow.Layers {
		layersByNumber[layer.Layer] = layer.UserID
	}
	require.Equal(t, userB.ID.String(), layersByNumber[1])
	require.Equal(t, userC.ID.String(), layersByNumber[2])
}
...
func TestGraphQLOnCallNowRequiresAdmin(t *testing.T) {
	...
	require.NotNil(t, allowedResp.Data.OnCallNow)
	require.Equal(t, schedule.ID.String(), allowedResp.Data.OnCallNow.ScheduleID)
}
