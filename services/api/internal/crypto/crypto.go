...
const (
	// DefaultKeyID is the key identifier for secrets encrypted with ESCALITE_ENCRYPTION_KEY.
	// Future key rotation stores a new id alongside ciphertext without re-encrypting all rows at once.
	DefaultKeyID = "v1"
...
// Decrypt opens ciphertext encrypted by this box. Returns an actionable error when key ids differ.
func (b *Box) Decrypt(enc Encrypted) ([]byte, error) {
	if enc.KeyID != b.keyID {
		return nil, &KeyIDMismatchError{
			Configured: b.keyID,
			Stored:     enc.KeyID,
		}
	}
