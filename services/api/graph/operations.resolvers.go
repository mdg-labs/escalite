...
func (r *mutationResolver) UpdateOrganization(ctx context.Context, input model.UpdateOrganizationInput) (*model.Organization, error) {
	...
	queries := db.New(r.pool)
	org, err := queries.UpdateOrganization(ctx, db.UpdateOrganizationParams{
		ID:   sc.User.OrganizationID,
		Name: name,
	})
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, gqlerr.New(handlers.CodeNotFound, "organization not found")
		}
		r.logger.Error("update organization failed", "error", err)
		return nil, gqlerr.New(handlers.CodeInternal, "internal error")
	}

	r.audit.OrganizationUpdated(ctx, queries, sc.User.OrganizationID, sc.User.ID)
