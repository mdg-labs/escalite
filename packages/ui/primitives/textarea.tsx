...
export function Textarea({
  ...
  const textareaClassName = cn(
    'w-full min-w-0 resize-y rounded-[inherit] bg-transparent px-[calc(--spacing(3)-1px)] py-[calc(--spacing(2)-1px)] outline-none placeholder:text-muted-foreground/72 sm:text-sm',
    size === 'sm' && 'px-[calc(--spacing(2.5)-1px)] py-[calc(--spacing(1.5)-1px)] text-sm',
    size === 'lg' && 'py-[calc(--spacing(2.5)-1px)] text-base',
    !unstyled && 'min-h-24',
    className,
  )
  ...
  return (
    ...
    >
      <textarea className={textareaClassName} data-slot="textarea" {...props} />
    </span>
  )
}
